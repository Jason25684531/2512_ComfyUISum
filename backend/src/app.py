...
from config import (
    ...
    STORAGE_OUTPUT_DIR, STORAGE_INPUT_DIR,
...
UPLOAD_FOLDER = STORAGE_INPUT_DIR
    ...
    try:
        ...
        if VEO3_TEST_MODE and workflow == 'veo3_long_video':
            ...
            if has_images:
                ...
                redis_client.expire(status_key, JOB_STATUS_EXPIRE_SECONDS)
...
def api_health():
    """Alias for /health to support ingress /api prefix."""
    return health()
...
def serve_output(filename):
    ...
    # 使用統一的 STORAGE_OUTPUT_DIR 配置（相容容器路徑）
    outputs_dir = str(STORAGE_OUTPUT_DIR)
