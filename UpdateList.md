# 更新日誌 (Update List)

---

## 📅 2026-01-02: 程式碼清理與架構整理

### 🧹 代碼清理

| 項目 | 類型 | 說明 |
|------|------|------|
| `frontend/app.js` | ❌ 已刪除 | 功能已整合到 index.html 內的 `<script>` 區塊 |
| `frontend/test_navigation.html` | ❌ 已刪除 | 開發期間的導航測試頁面，已不再需要 |
| `frontend/diagnostic.html` | ❌ 已刪除 | 開發期間的診斷面板，已不再需要 |
| `backend/src/__pycache__/` | ❌ 已刪除 | Python 編譯快取 |
| `worker/src/__pycache__/` | ❌ 已刪除 | Python 編譯快取 |
| `backend.log` | ❌ 已刪除 | 根目錄日誌檔案 |

### 📁 目前專案結構

```
ComfyUISum/
├── backend/                     # Flask API 服務 (Port 5000)
│   ├── Dockerfile
│   ├── requirements.txt
│   ├── Readmd/                  # API 文檔
│   └── src/
│       ├── app.py              # API 端點 (generate, status, cancel, models, history)
│       ├── config.py           # Backend 配置
│       └── database.py         # MySQL 資料庫操作 (連接池, Jobs 表)
│
├── worker/                      # 任務處理服務
│   ├── Dockerfile
│   ├── requirements.txt
│   └── src/
│       ├── main.py             # Worker 主迴圈 (Redis 監聽, 任務處理)
│       ├── comfy_client.py     # ComfyUI API 客戶端 (WebSocket 進度)
│       ├── json_parser.py      # Workflow JSON 解析器 (參數注入)
│       ├── config.py           # Worker 配置
│       └── check_comfy_connection.py  # 診斷工具
│
├── frontend/                    # Web UI (Single Page Application)
│   ├── index.html              # 主頁面 (含所有 JS 邏輯)
│   └── style.css               # 樣式表
│
├── ComfyUIworkflow/             # ComfyUI Workflow 模板
│   ├── text_to_image_*.json
│   ├── face_swap_*.json
│   ├── multi_image_blend_*.json
│   ├── single_image_edit_*.json
│   └── sketch_to_image_*.json
│
├── storage/                     # 本地儲存
│   ├── inputs/                 # 暫存上傳圖片 (24h)
│   ├── outputs/                # 生成結果 (30天)
│   └── models/
│
├── openspec/                    # 專案規格文檔
│   ├── project.md              # 專案說明
│   ├── AGENTS.md
│   ├── specs/                  # 模組規格
│   └── changes/                # 變更記錄
│
├── docker-compose.yml           # Docker 編排 (Redis + MySQL + Backend + Worker)
├── docker-compose.dev.yml       # 開發環境配置
├── .env                         # 環境變數
├── .gitignore                   # Git 忽略規則
├── requirements.txt             # Python 依賴 (根目錄)
├── README.md                    # 專案說明
├── STARTUP_GUIDE.md             # 啟動指南
└── start_all.bat               # 一鍵啟動腳本
```

### 🔌 API 端點清單

| 方法 | 端點 | 功能 | 說明 |
|------|------|------|------|
| POST | `/api/generate` | 提交生成任務 | 支援 5 種工作流 |
| GET | `/api/status/{job_id}` | 查詢任務狀態 | 含進度百分比 |
| POST | `/api/cancel/{job_id}` | 取消任務 | 發送中斷指令到 ComfyUI |
| GET | `/api/models` | 獲取模型列表 | 掃描 checkpoints + unet |
| GET | `/api/history` | 獲取歷史記錄 | 支援分頁 (limit/offset) |
| GET | `/health` | 健康檢查 | 回傳 Redis + MySQL 狀態 |
| GET | `/outputs/{filename}` | 獲取生成圖片 | 靜態檔案服務 |

### 🔄 資料流程

```
[Frontend] → POST /api/generate → [Backend] → Redis Queue
                                       ↓
                              [Worker] ← BLPOP Redis
                                       ↓
                              Parse Workflow → [ComfyUI]
                                       ↓
                              WebSocket 監聽進度
                                       ↓
                              複製輸出圖片 → storage/outputs/
                                       ↓
                              更新 Redis 狀態 + MySQL 記錄
                                       ↓
[Frontend] ← GET /api/status ← [Backend] ← Redis
```

### 📋 下一步展望 (Phase 4 規劃)

#### 優先級 1: 穩定性與日誌
- [ ] Worker 錯誤日誌寫入檔案 (`logs/worker.log`)
- [ ] Backend 錯誤日誌寫入檔案 (`logs/backend.log`)
- [ ] API 請求日誌 (request/response)

#### 優先級 2: 功能增強
- [ ] Prompt 模板系統 - 預設風格選擇
- [ ] 圖片收藏功能 - 標記喜歡的圖片
- [ ] 進階參數 - CFG Scale, Steps, Sampler 選項

#### 優先級 3: 性能優化
- [ ] 圖片壓縮 - 上傳前自動壓縮大圖
- [ ] 結果快取 - 相同參數直接返回快取結果
- [ ] 併發控制 - 限制同時處理的任務數量

#### 優先級 4: 部署完善
- [ ] HTTPS 支援 - 使用 Nginx 反向代理
- [ ] 用戶認證 - 簡易 API Key 認證
- [ ] 監控面板 - 任務統計和系統狀態
- [ ] Ngrok 整合 - 內網穿透測試

---

## 📅 2026-01-02: Phase 3 - Data & Intelligence 完成

### 🎯 今日完成事項 (Phase 3 全部完成)

| 模組 | 功能 | 狀態 |
|------|------|------|
| **Database** | MySQL 8.0 資料庫建置 | ✅ 完成 |
| **Database** | Jobs 表 Schema 設計 | ✅ 完成 |
| **Backend** | 資料庫連接池整合 | ✅ 完成 |
| **Backend** | `GET /api/history` 歷史記錄 API | ✅ 完成 |
| **Backend** | 任務狀態同步到資料庫 | ✅ 完成 |
| **Frontend** | Personal Gallery 頁面 | ✅ 完成 |
| **Frontend** | Remix 功能（重新使用參數） | ✅ 完成 |
| **Worker** | 資料庫同步清理（Soft Delete） | ✅ 完成 |
| **Worker** | ComfyUI 錯誤重試機制 | ✅ 完成 |
| **Docker** | MySQL 服務整合 | ✅ 完成 |
| **Docker** | Backend Healthcheck | ✅ 完成 |
| **Docker** | 鏡像大小優化 | ✅ 完成 |

### 🗄️ 新增資料庫架構

#### Jobs 表結構
```sql
CREATE TABLE jobs (
    id VARCHAR(36) PRIMARY KEY,              -- Job UUID
    prompt TEXT,                             -- 提示詞
    workflow VARCHAR(50),                    -- 工作流類型
    model VARCHAR(100),                      -- 使用模型
    aspect_ratio VARCHAR(10),                -- 圖片比例
    batch_size INT DEFAULT 1,                -- 批次數量
    seed INT DEFAULT -1,                     -- 隨機種子
    status VARCHAR(20),                      -- 狀態 (queued, processing, finished, failed)
    output_path TEXT,                        -- 輸出路徑 (多張用逗號分隔)
    created_at TIMESTAMP DEFAULT NOW,        -- 建立時間
    updated_at TIMESTAMP ON UPDATE NOW,      -- 更新時間
    is_deleted BOOLEAN DEFAULT FALSE,        -- 軟刪除標記
    INDEX idx_status (status),
    INDEX idx_created_at (created_at),
    INDEX idx_is_deleted (is_deleted)
);
```

### 🆕 新增 API 端點

| 方法 | 端點 | 功能 | 參數 |
|------|------|------|------|
| GET | `/api/history` | 獲取歷史記錄 | `limit` (預設 50), `offset` (預設 0) |
| GET | `/health` | 健康檢查（強化版） | - (回傳 Redis + MySQL 狀態) |

### 🎨 新增前端功能

#### Personal Gallery
- **響應式網格佈局**: 1-4 欄自適應顯示
- **卡片資訊**: 縮圖、提示詞、工作流類型、日期、狀態
- **圖片數量標記**: 批次生成時顯示圖片數
- **狀態顏色編碼**: 
  - ✅ Finished (綠色)
  - ❌ Failed (紅色)
  - ⏳ Processing (黃色)
  - 📋 Queued (藍色)
  - 🚫 Cancelled (灰色)

#### Remix 功能
點擊歷史記錄的 "Remix" 按鈕：
1. 自動跳轉到 Image Composition 工作區
2. 選擇對應的工作流工具
3. 填充原始參數（Prompt、Seed、Model、Aspect Ratio）
4. 允許快速修改並重新生成

### 🔧 系統韌性強化

#### Worker 自動維護
- **啟動時清理**: 刪除過期檔案並同步資料庫
- **定期清理**: 每小時執行一次
- **軟刪除機制**: 刪除檔案時標記資料庫 `is_deleted = TRUE`，保留歷史記錄

#### 錯誤重試機制
- **ComfyUI 連接重試**: 失敗時等待 5 秒後重試 1 次
- **Redis 斷線重連**: 自動重連並繼續處理佇列

### 🐳 Docker 優化

#### MySQL 服務
- **版本**: MySQL 8.0
- **端口**: 3306 (本地 DBeaver 可連接)
- **持久化**: `./mysql_data:/var/lib/mysql`
- **Healthcheck**: 確保資料庫就緒後才啟動 Backend

#### 鏡像優化
- **Python 版本**: 升級到 3.10-slim
- **Pip 優化**: `--no-cache-dir` 減少鏡像大小
- **Backend Healthcheck**: 每 30 秒檢查 `/health` 端點

### 📦 依賴更新

#### Backend
```txt
+ mysql-connector-python==8.2.0
```

### 🔄 資料流程圖

```
┌──────────┐     ┌──────────┐     ┌──────────┐
│  Browser │────▶│  Backend │────▶│   MySQL  │
│          │     │   API    │     │          │
└──────────┘     └────┬─────┘     └──────────┘
     ▲                │                 ▲
     │                ▼                 │
     │          ┌──────────┐            │
     │          │  Redis   │            │
     │          │  Queue   │            │
     │          └────┬─────┘            │
     │               │                  │
     │               ▼                  │
     │          ┌──────────┐       Soft Delete
     │          │  Worker  │            │
     │          │          ├────────────┘
     │          └────┬─────┘
     │               │
     │               ▼
     │          ┌──────────┐
     └──────────│ ComfyUI  │
                │          │
                └──────────┘
```

### 📝 環境變數更新

#### .env 新增配置
```ini
# MySQL Database Configuration
DB_HOST=localhost
DB_PORT=3306
DB_USER=studio_user
DB_PASSWORD=studio_password
DB_NAME=studio_db
```

### 🧪 測試驗證步驟

1. **啟動完整堆疊**
   ```bash
   docker-compose up -d
   ```

2. **檢查健康狀態**
   ```bash
   curl http://localhost:5000/health
   # 應回傳: {"status":"ok","redis":"healthy","mysql":"healthy"}
   ```

3. **生成測試圖片**
   - 提交任務並確認資料庫有記錄

4. **訪問 Personal Gallery**
   - 確認能看到歷史記錄
   - 測試 Remix 功能

5. **測試清理機制**
   - 修改檔案時間戳模擬過期檔案
   - 重啟 Worker 確認自動清理

### 🚀 部署提醒

1. **資料庫初始化**: 首次啟動時 Backend 會自動建立 `jobs` 表
2. **資料持久化**: 確保 `./mysql_data` 目錄有寫入權限
3. **本地開發**: Backend 和 Worker 需要手動設定 `DB_HOST=localhost`
4. **Docker 環境**: 自動使用 `DB_HOST=mysql` 透過內部網路連接

---

## 📅 2024-12-31: 今日總結與下週展望

### 🎯 今日完成事項 (Phase 2 全部完成)

| 模組 | 功能 | 狀態 |
|------|------|------|
| **UX** | 即時進度條 | ✅ 完成 |
| **API** | 動態模型列表 `GET /api/models` | ✅ 完成 |
| **API** | 任務取消 `POST /api/cancel/{job_id}` | ✅ 完成 |
| **UI** | 動態模型下拉選單 | ✅ 完成 |
| **UI** | 批次圖片網格顯示 | ✅ 完成 |
| **Docker** | Backend + Worker 容器化 | ✅ 完成 |
| **Docker** | docker-compose 完整編排 | ✅ 完成 |
| **Worker** | 圖片自動清理 (30天) | ✅ 完成 |
| **Worker** | 輸出圖片智能選擇 | ✅ 完成 |
| **Config** | ComfyUI 路徑環境變數 | ✅ 完成 |

### 🛠️ 今日修復的 Bug

1. **WebSocket 解析錯誤** - `'NoneType' object has no attribute 'get'`
   - **原因**: ComfyUI 回傳的 `data` 欄位為 `null`
   - **修復**: 在 `comfy_client.py` 添加類型檢查

2. **輸出圖片找不到** - ComfyUI 輸出多張圖片時複製失敗
   - **原因**: 舊代碼只保留最後一張圖片信息
   - **修復**: 收集所有圖片，優先選擇有 subfolder 的正式輸出

3. **ComfyUI 路徑錯誤** - Worker 找不到輸出目錄
   - **原因**: 預設路徑假設錯誤
   - **修復**: 更新 `.env` 配置正確的 ComfyUI 路徑

### 🧹 代碼清理

- ❌ 刪除 `backend/src/routes.py` (空文件)
- ❌ 刪除 `__pycache__/` 目錄
- ❌ 刪除 `backend.log` 日誌
- ❌ 刪除 `restart_worker.ps1` (臨時腳本)
- ✅ 更新 `.gitignore` 規則

### 📊 當前架構

```
ComfyUISum/
├── backend/                    # Flask API 服務
│   ├── Dockerfile
│   ├── requirements.txt
│   └── src/
│       ├── app.py             # API 端點 (generate, status, cancel, models)
│       └── config.py          # Backend 配置
│
├── worker/                     # 任務處理服務
│   ├── Dockerfile
│   ├── requirements.txt
│   └── src/
│       ├── main.py            # Worker 主迴圈
│       ├── comfy_client.py    # ComfyUI API 客戶端
│       ├── json_parser.py     # Workflow 解析器
│       ├── config.py          # Worker 配置
│       └── check_comfy_connection.py  # 診斷工具
│
├── frontend/                   # Web UI
│   ├── index.html             # 單頁應用
│   ├── style.css
│   └── app.js
│
├── storage/                    # 本地儲存
│   ├── inputs/                # 暫存上傳圖片 (24h)
│   └── outputs/               # 生成結果 (30天)
│
├── docker-compose.yml          # 容器編排
├── .env                        # 環境變數
├── DEPLOYMENT.md               # 部署指南
└── ARCHITECTURE.md             # 架構說明
```

### 🔌 API 端點清單

| 方法 | 端點 | 功能 |
|------|------|------|
| POST | `/api/generate` | 提交生成任務 |
| GET | `/api/status/{job_id}` | 查詢任務狀態 |
| POST | `/api/cancel/{job_id}` | 取消任務 |
| GET | `/api/models` | 獲取模型列表 |
| GET | `/outputs/{filename}` | 獲取生成圖片 |
| GET | `/health` | 健康檢查 |

---

### 📋 下週展望 (Phase 3 規劃)

#### 優先級 1: 穩定性與測試
- [ ] 完善單圖編輯 (single_image_edit) 工作流測試
- [ ] 修復所有 Workflow 類型的端到端測試
- [ ] 添加錯誤重試機制 (任務失敗自動重試 1 次)
- [ ] 添加日誌記錄到文件 (Worker + Backend)

#### 優先級 2: 功能增強
- [ ] 歷史記錄功能 - 保存最近 50 筆生成記錄
- [ ] 圖片收藏功能 - 標記喜歡的圖片
- [ ] Prompt 模板 - 預設風格模板選擇
- [ ] 進階參數 - CFG Scale, Steps, Sampler 選項

#### 優先級 3: 性能優化
- [ ] 圖片壓縮 - 上傳前自動壓縮大圖
- [ ] 結果快取 - 相同參數直接返回快取結果
- [ ] 併發控制 - 限制同時處理的任務數量

#### 優先級 4: 部署完善
- [ ] HTTPS 支援 - 使用 Nginx 反向代理
- [ ] 用戶認證 - 簡易 API Key 認證
- [ ] 監控面板 - 任務統計和系統狀態

---

## 2024-12-31: Phase 2 - Step 4: 任務管理與維護

### 概述
實現任務取消功能和圖片自動清理機制，提升系統的可控性和磁碟空間管理能力。

### 變更內容

#### 1. Backend API - `backend/src/app.py`
- **新增 `POST /api/cancel/{job_id}` 端點**
  - 檢查任務是否存在
  - 驗證任務狀態（只能取消 `queued` 或 `processing` 狀態的任務）
  - 將任務狀態設置為 `cancelled`
  - 設置錯誤訊息：`Task cancelled by user`
  - 回應格式：
    ```json
    {
      "success": true,
      "message": "Task cancelled"
    }
    ```

#### 2. Worker - `worker/src/comfy_client.py`
- **新增 `interrupt()` 方法**
  - 發送 `POST /interrupt` 請求到 ComfyUI
  - 中斷當前執行的任務
  - 返回布爾值表示是否成功

#### 3. Worker - `worker/src/main.py`
- **`process_job()` 函數增強**
  - 在 `on_progress` 回調中檢查任務狀態
  - 如果狀態變為 `cancelled`，立即調用 `client.interrupt()`
  - 拋出異常終止任務處理

- **新增 `cleanup_old_output_files()` 函數**
  - 掃描 `storage/outputs` 目錄
  - 刪除超過 30 天的圖片檔案
  - 記錄刪除數量和釋放空間

- **主循環增強**
  - Worker 啟動時執行一次圖片清理
  - 每小時定期執行圖片清理（包括暫存檔案和輸出圖片）

### 技術細節

#### 任務取消流程
```
用戶請求 → POST /api/cancel/{job_id} → 
Redis 狀態更新為 'cancelled' → 
Worker 檢測到狀態變化 → 
調用 ComfyUI /interrupt → 
任務終止
```

#### 圖片清理流程
```
Worker 啟動 → cleanup_old_output_files() → 
每小時觸發 → 刪除超過 30 天的檔案 → 
記錄清理統計
```

### 清理策略
- **暫存檔案**（`storage/inputs`）: 保留 24 小時
- **輸出圖片**（`storage/outputs`）: 保留 30 天
- **清理頻率**: 每小時檢查一次

### 驗證步驟

#### 測試任務取消
1. 提交一個生成任務
2. 在處理過程中調用 `POST /api/cancel/{job_id}`
3. 確認任務狀態變為 `cancelled`
4. 確認 ComfyUI 收到中斷指令

#### 測試圖片清理
1. 在 `storage/outputs` 放置舊檔案（修改時間戳）
2. 啟動 Worker
3. 檢查舊檔案是否被刪除
4. 查看日誌確認清理統計

### 相關文件
- [task.md](openspec/changes/phase-2-maturity/task.md) - Phase 2 任務清單

---

## 2024-12-31: Phase 2 - Step 3: Docker 容器化部署

### 概述
完善 Docker 容器化配置，實現一鍵啟動整個系統（Redis、Backend、Worker）。

### 變更內容

#### 1. Backend - `backend/Dockerfile`
- **基礎映像**: Python 3.9 Slim
- **工作目錄**: `/app`
- **複製檔案**: `requirements.txt` + `src/`
- **啟動命令**: `python src/app.py`

#### 2. Worker - `worker/Dockerfile`
- **基礎映像**: Python 3.9 Slim
- **工作目錄**: `/worker`
- **複製檔案**: `requirements.txt` + `src/`
- **啟動命令**: `python src/main.py`

#### 3. Docker Compose - `docker-compose.yml`
- **Redis 服務**:
  - 映像: `redis:7.2`
  - 端口: `6379`
  - 持久化: `./redis_data:/data`
  - 密碼: `mysecret`
  
- **Backend 服務**:
  - 建置: `./backend`
  - 端口: `5000:5000`
  - 環境變數: Redis 連接配置
  - Volume: `./storage/outputs`
  
- **Worker 服務**:
  - 建置: `./worker`
  - 環境變數: Redis + ComfyUI 連接配置
  - Volume: `./storage`
  - ComfyUI 連接: `host.docker.internal:8188`

### 使用方式

#### 啟動所有服務
```bash
docker-compose up -d
```

#### 查看日誌
```bash
docker-compose logs -f backend
docker-compose logs -f worker
```

#### 停止所有服務
```bash
docker-compose down
```

### 注意事項

1. **ComfyUI 必須在宿主機運行**
   - Worker 透過 `host.docker.internal` 連接宿主機的 ComfyUI
   - 確保 ComfyUI 已啟動在 `http://127.0.0.1:8188`

2. **Volume 掛載**
   - `./storage/outputs`: 生成的圖片輸出目錄
   - `./redis_data`: Redis 持久化數據

3. **環境變數**
   - 可透過 `.env` 檔案或 `docker-compose.yml` 覆寫
   - 重要配置: `COMFY_HOST`, `REDIS_PASSWORD`

### 相關文件
- [task.md](openspec/changes/phase-2-maturity/task.md) - Phase 2 任務清單

---

## 2024-12-31: Phase 2 - Step 2: 動態配置與批次顯示

### 概述
實現動態模型列表獲取和批次圖片網格顯示功能，讓前端能夠自動掃描 ComfyUI 的模型目錄，並優雅地展示多張生成圖片。

### 變更內容

#### 1. Backend - `backend/src/config.py`
- **新增 ComfyUI 路徑配置**
  - `COMFYUI_ROOT`: ComfyUI 安裝根目錄
  - `COMFYUI_MODELS_DIR`: 模型總目錄
  - `COMFYUI_CHECKPOINTS_DIR`: Checkpoint 模型目錄
  - `COMFYUI_UNET_DIR`: UNET 模型目錄

#### 2. Backend API - `backend/src/app.py`
- **新增 `GET /api/models` 端點**
  - 掃描 `checkpoints` 目錄，查找 `.safetensors` 和 `.ckpt` 檔案
  - 掃描 `unet` 目錄，查找 `.safetensors`、`.ckpt`、`.pt` 檔案
  - 使用相對路徑（支援子目錄）
  - 錯誤處理：目錄不存在時返回預設模型列表
  - 回應格式：
    ```json
    {
      "models": ["model1.safetensors", "model2.ckpt"],
      "unet_models": ["z-image/z-image-turbo-fp8.safetensors"]
    }
    ```

#### 3. Frontend - `frontend/index.html`
- **動態模型載入功能**
  - 移除硬編碼的 `<option>` 標籤
  - 新增 `fetchModels()` 函數：
    - 調用 `GET /api/models` API
    - 分組顯示 UNET 和 Checkpoint 模型
    - 錯誤處理：API 失敗時顯示預設選項
  - 在 `DOMContentLoaded` 時自動調用

- **批次圖片網格顯示驗證**
  - `showResult()` 函數已正確實現：
    - 1張圖：`grid-cols-1` (單欄置中)
    - 2張圖：`grid-cols-2` (雙欄)
    - 3-4張圖：`grid-cols-2 grid-rows-2` (2x2 網格)
  - 每張圖片獨立的下載按鈕
  - 響應式設計，支援不同螢幕尺寸

### 技術細節

#### 模型掃描流程
```
ComfyUI/models/checkpoints → Backend (遞迴掃描) → API → Frontend (動態填充下拉選單)
ComfyUI/models/unet → Backend (遞迴掃描) → API → Frontend (動態填充下拉選單)
```

#### 批次圖片顯示邏輯
```javascript
// 根據圖片數量動態調整 Grid 布局
if (count === 1) {
    resultsGrid.className = 'grid-cols-1';
} else if (count === 2) {
    resultsGrid.className = 'grid-cols-2';
} else {
    resultsGrid.className = 'grid-cols-2 grid-rows-2';
}
```

### 驗證步驟
1. 確保 ComfyUI 已安裝模型
2. 啟動 Backend API
3. 打開前端頁面
4. 檢查模型下拉選單是否顯示實際模型
5. 設定 Batch Size > 1 測試多圖顯示

### 相關文件
- [task.md](openspec/changes/phase-2-maturity/task.md) - Phase 2 任務清單

---

## 2024-12-31: Phase 2 - Step 1: 即時進度條實裝

### 概述
實現即時進度反饋功能，讓使用者在生成圖片時能看到實際進度百分比，而非只有靜態的 "Processing..." 訊息。

### 變更內容

#### 1. Worker - `worker/src/comfy_client.py`
- **`wait_for_completion()` 方法增強**
  - 新增 `on_progress` 回調函數參數
  - 監聽 ComfyUI WebSocket 的 `progress` 事件
  - 解析 `{'type': 'progress', 'data': {'value': X, 'max': Y}}` 訊息
  - 計算進度百分比：`int((value / max) * 100)`
  - 透過回調函數通知進度更新

#### 2. Worker - `worker/src/main.py`
- **`process_job()` 函數重構**
  - 修正執行流程：`queue_prompt()` → `wait_for_completion()` → `copy_output_image()`
  - 新增 `on_progress` 回調函數，將進度映射到 30-95% 區間
  - 修正任務完成狀態為 `finished`（與前端一致）
  - 新增輸出圖片複製邏輯

#### 3. Backend API - `backend/src/app.py`
- **`GET /api/status/<job_id>` 端點**
  - 已正確實現，返回 `progress` 欄位
  - 回應格式：`{"status": "processing", "progress": 50, "image_url": "", "error": ""}`

#### 4. Frontend - `frontend/index.html`
- **`pollStatus()` 函數**
  - 已正確實現，讀取 `data.progress` 並顯示
  - 狀態訊息格式：`🔄 Processing... {progress}%`

### 技術細節

#### 進度計算流程
```
ComfyUI WebSocket → Worker (comfy_client.py) → Redis (job:status:<job_id>) → API → Frontend
```

#### 進度映射
- 10%: 任務開始處理
- 15%: 處理上傳圖片
- 20%: 解析 Workflow
- 30%: 提交任務到 ComfyUI
- 30-95%: ComfyUI 執行中（由 `progress` 事件動態更新）
- 100%: 任務完成

### 驗證步驟
1. 啟動 Redis、Backend、Worker 服務
2. 啟動 ComfyUI
3. 在前端提交生成請求
4. 觀察狀態訊息是否顯示動態進度百分比

### 相關文件
- [task.md](openspec/changes/phase-2-maturity/task.md) - Phase 2 任務清單
